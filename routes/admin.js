...
router.put("/takeAction", async(req, res) => {
    ...
    let newAlert = {
        ...
        subject: req.body.userReq.subject ||
            req.body.userReq.loanReason ||
            req.body.userReq.bonusReason,
        reason: req.body.userReq.reason ||
    ...
    User.findOneAndUpdate({ _id: req.body.userReq.empId }, { notification: updatedNotificationList, alert: alert }, { new: true },
        function(err, result) {
    ...
    if (req.body.userReq.title === "leave request") {
        ...
        Admin.findByIdAndUpdate(
            req.body.adminId, { leaveRequests: updatedLeaveReq }, { new: true },
            function(err, result) {
                if (err) res.status(400).json("Error: ", err);
        ...
        if (isApproved) {
            ...
            await Salary.findOneAndUpdate({ empId: req.body.userReq.empId }, { totalLeaves: currentLeaves }, { new: true });
        }
        ...
        Admin.findByIdAndUpdate(
            req.body.adminId, { bonusRequests: updatedBonusReq }, { new: true },
            function(err, result) {
                if (err) res.status(400).json("Error: ", err);
                else res.json(result);
            }
        ...
        try {
            ...
            Admin.findByIdAndUpdate(
                req.body.adminId, { loanRequests: updatedLoanReq }, { new: true },
                function(err, result) {
                    if (err) res.status(400).json("Error: ", err);
                }
            );
...
router.put("/generateSalReceipt", async(req, res) => {
    try {
        ...
        await Salary.findOneAndUpdate({ empId: req.body.empId }, { bonus: 0, totalLeaves: 0 });
        ...
        const updatedEmpReceiptDoc = await SalaryReceipt.findOneAndUpdate({ empId: req.body.empId }, {
...
router.put("/updateSalaryDetails/:id", async(req, res) => {
    try {
        ...
        const salDoc = await Salary.findOneAndUpdate({ empId: req.params.id }, {
        ...
        const salReceipt = await SalaryReceipt.findOneAndUpdate({ empId: req.params.id }, {
